...
设置完成后点击"保存设置"按钮，这些设置将同时保存在浏览器本地和服务器端，确保在任何地方访问系统时都使用相同的设置。
...
- **优化交易逻辑**：修复了无法正确平仓的问题
- **改进交易规则**：
  - 空仓状态：收到买入信号开多单，收到卖出信号开空单
  - 持有多单：收到买入信号拒绝，收到卖出信号平多单（不再自动开空）
  - 持有空单：收到买入信号平空单（不再自动开多），收到卖出信号拒绝
- **改进错误处理**：添加了更详细的错误日志和提示
- **优化持仓管理**：改进了持仓状态的管理逻辑
